...
namespace ProDriver.Driver
{
    ...
    /// 日期:200-06-20]
    /// </summary>
    public class CameraDriver_MindVision : CameraDriver
    {
        ...
        protected override bool DoOpen()
        {
            ...
            try
            {
                ...
                {
                    //获取相机的特性描述
                    status=MVSDK.MvApi.CameraGetCapability(_deviceRef, out _deviceCapacity);
                    rt = (MVSDK.CameraSdkStatus.CAMERA_STATUS_SUCCESS == status) ? true : false;
                    ...
                    if(rt)
                        ...
                        {
                            //黑白相机输出8位灰度图数据
                            status=MVSDK.MvApi.CameraSetIspOutFormat(_deviceRef, (uint)MVSDK.emImageFormat.CAMERA_MEDIA_TYPE_MONO8);
                            rt = (MVSDK.CameraSdkStatus.CAMERA_STATUS_SUCCESS == status) ? true : false;
                        }
                ...
                if (_deviceRef > 0)
                {
                    switch (acqmode)
                    {
                        ...
                        case ProCommon.Communal.AcquisitionMode.ExternalTrigger:
                            ...
                            //if(rt)
                            //{
                            //    if (DoSetTriggerDelay(0, 1))
                            //        rt = DoSetDebouncerTime(0, 50);
                            //}
                            break;
                        default: break;
                    }
