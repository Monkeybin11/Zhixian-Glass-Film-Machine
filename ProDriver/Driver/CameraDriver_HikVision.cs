...
namespace ProDriver.Driver
{
    ...
    /// 日期:200-06-20]
    /// </summary>
    public class CameraDriver_HikVision : CameraDriver
    {
        //----------------------------自定义错误代码-----------------------------------//
        //99-图像处理类未实例化
        //98-设备列表为空
        ...
        public CameraDriver_HikVision(ProCommon.Communal.CameraProperty cam) : base(cam)
        {
        ...
        protected override bool DoSetGamma(float gamma)
        {
            ...
            try
            {
                if (this._deviceRef != null)
                {
                    nRet = _deviceRef.MV_CC_SetBoolValue_NET("GammaEnable", true);
                    rt = (MvCamCtrl.NET.MyCamera.MV_OK == nRet) ? true : false;

                    if(rt)
                        nRet = _deviceRef.MV_CC_SetFloatValue_NET("Gamma", gamma);
                }
                ...
                {
                    if (DriverExceptionDel != null)
                        DriverExceptionDel(string.Format("错误：海康相机设置伽马失败!\n错误代码:{0:X8}", nRet));
                }
            }
            catch
            {
