...
namespace ProDriver.Driver
{
    ...
    /// 日期:200-06-20]
    /// </summary>
    public class CameraDriver_Baumer : CameraDriver
    {
        ...
        public CameraDriver_Baumer(ProCommon.Communal.CameraProperty cam) : base(cam)
        {
        ...
        private void OnCameraImageGrabbed(object sender, NewBufferEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    BitmapToHObject(Bitmap, out HoImage);//Bitmap转换为HObject
        ...
        protected override bool DoSetTriggerDelay(int idx,float trigdelay)
        {
            ...
            try
            {
                if (_bdevice != null)
                {
                    //SDK--目前相机固定触发输入,无需指定输入端口,时间单位未知?
        ...
        protected override bool DoSetDebouncerTime(int idx,float debouncertime)
        {
            ...
            try
            {
                if (_bdevice != null)
                {
                    //SDK--目前相机固定触发输入,无需指定输入端口;且消抖字符串未知
                    //_bdevice.RemoteNodeList["TriggerDelay"].Value = (double)debouncertime;
                    rt = true;
                }
            }
            catch (BGAPI2.Exceptions.IException ex)
            {
                ...
                //MessageBox.Show(str18, "错误提示", MessageBoxButtons.OK, MessageBoxIcon.Error);             
                if (DriverExceptionDel != null)
                    DriverExceptionDel(string.Format("错误：堡盟相机设置消抖时间失败!\n异常类型:{0}\n异常描述:{1}", ex.GetType(), ex.GetErrorDescription()));
            }
            return rt;
        }
        ...
        protected override bool DoGetInPut(int idx, out bool onOff)
        {
            ...
            catch (BGAPI2.Exceptions.IException ex)
            {
                ...
                //MessageBox.Show(str18, "错误提示", MessageBoxButtons.OK, MessageBoxIcon.Error);             
                if (DriverExceptionDel != null)
                    DriverExceptionDel(string.Format("错误：堡盟相机获取输入端口状态失败!\n异常类型:{0}\n异常描述:{1}", ex.GetType(), ex.GetErrorDescription()));
            }
            return rt;
        }
        ...
        protected override bool DoSetOutPut(int idx, bool onOff)
        {
            ...
            {
                if (DriverExceptionDel != null)
                    DriverExceptionDel(string.Format("错误：堡盟相机设置输出端口状态失败!\n错误描述:{0}",ex.Message));
            }
            finally
            {
            }
        ...
        protected override bool DoGetCameraConnectedState(out bool isConnected)
        {
            ...
            {
                if (DriverExceptionDel != null)
                    DriverExceptionDel(string.Format("错误：堡盟相机获取连接状态失败!\n错误描述:{0}", ex.Message));
            }
            finally
            {
            }
