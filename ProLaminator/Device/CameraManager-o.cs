...
namespace ProLaminator.Device
{
    ...
    public class CameraManager:ProCommon.Communal.DeviceManager
    {
        ...
        private const int MAX_IMG_BUFFERS = 10;
                ...
                */
                    ...
                    && img.IsInitialized())
                {
                    ...
                    if (resultOK)
                    {
                        ...
                        dx = (_imgProcessForGlass.Row - imgHeight).D * CamForGlass.CalSolution.RowUnit;
                        x = Convert.ToSingle(System.Math.Round(dx, 2));

                        dy = (_imgProcessForGlass.Col - imgWidth).D * CamForGlass.CalSolution.ColUnit;                     
            ...
            while (_arsteNewImgForMembrane1.WaitOne())
            {
                ...
                */
                    ...
                    && img.IsInitialized())
                {
                    ...
                    {
                        //当前项目:像素Row与世界坐标系X平齐,像素Col与世界坐标系Y平齐
                        dx = (_imgProcessForMembrane1.Row - imgHeight).D * CamForMembrane1.CalSolution.RowUnit;
                        x = Convert.ToSingle(System.Math.Round(dx, 2));

                        dy = (_imgProcessForMembrane1.Col - imgWidth).D * CamForMembrane1.CalSolution.ColUnit;
                        y = Convert.ToSingle(System.Math.Round(dy, 2));
            ...
            while (_arsteNewImgForMembrane2.WaitOne())
            {
                ...
                */
                    ...
                    && img.IsInitialized())
                {
                    ...
                    {
                        //当前项目:像素Row与世界坐标系X平齐,像素Col与世界坐标系Y平齐
                        dx = (_imgProcessForMembrane2.Row - imgHeight).D * CamForMembrane2.CalSolution.RowUnit;
                        x = Convert.ToSingle(System.Math.Round(dx, 2));

                        dy = (_imgProcessForMembrane2.Col - imgWidth).D * CamForMembrane2.CalSolution.ColUnit;
                        y = Convert.ToSingle(System.Math.Round(dy, 2));
            ...
            try
            {
                  ...
                  && _cameraPropertyList.Count > 0)
                {
                    for (int i = 0; i < _cameraPropertyList.Count; i++)
                    {
                        ...
                        bool f;
                        CameraList[id].API.GetInPut(0, out f);

                        if(f)
                            CameraList[id].API.SetOutPut(0, ProCommon.Communal.EffectiveSignal.LogicTrue,10);
