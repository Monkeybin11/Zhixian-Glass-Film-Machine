...
namespace ProVision.MatchModel
{
    public partial class FrmMatchModel : Form
    {
        ...
        /// 构造函数:只传入模型保存路径
        /// </summary>
        /// <param name="saveModelPath"></param>
        public FrmMatchModel(string saveModelPath):this()
        {
            _saveModelPath = saveModelPath;
        }

        /// <summary>
        /// 构造函数:同时传入训练图像和模型保存路径
        /// </summary>
        /// <param name="trainImage"></param>
        /// <param name="saveModelPath"></param>
        public FrmMatchModel(HalconDotNet.HObject trainImage,string saveModelPath) : this()
        {
            TransmitTrainImg(trainImage);
            _saveModelPath = saveModelPath;
        }
        ...
        protected virtual void UpdateControls()
        {
            ...
            this.tlstrpmiResetWindow.Click -= Tlstrpmi_Click;
            this.tlstrpmiResetWindow.Click += Tlstrpmi_Click;
                    ...
                    && ExtractRegion.IsInitialized())
                {
                        ...
                        && MatchAssistant.ModelID.TupleNotEqual(new HalconDotNet.HTuple(-1)))
                    {
                        if (SearchRegion != null
                           && SearchRegion.IsInitialized())
                        {
                            string folderPath = _saveModelPath;

                            if (string.IsNullOrEmpty(_saveModelPath))
                            {
                                _fbdModelFolder.Description = "请选择或者创建一个文件夹";
                                if (_fbdModelFolder.ShowDialog() == DialogResult.OK)
                                    folderPath = _fbdModelFolder.SelectedPath;
                            }
        ...
        protected virtual void LoadModel()
        {
            ...
            if (_fbdModelFolder.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    this.chkbAlwaysFind.Checked = false;
                    if(MatchAssistant.LoadShapeModel(folderPath))
                    {
                        ModelPose = MatchAssistant.ModelPose;
