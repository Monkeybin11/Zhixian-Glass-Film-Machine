...
namespace ProVision.MatchModel
{
    ...
    public class ShapeModelAssistant
    {
        ...
        /// 训练图像的扩展名
        /// </summary>
        public string TrainImageExtension;
        ...
        /// </summary>
        /// <param name="filePath">模板参数文件目录路径</param>
        /// <returns></returns>
        public bool LoadShapeModel(string filePath)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(filePath))
                {
                    ...
                    //1--首先加载模型所需的训练图像(加载训练图像会把所有参数复位)
                    ...
                    if (fileExist.TupleNotEqual(new HalconDotNet.HTuple(0)))
                    {
                        ...
                        if ((dirInfo.GetFiles().Length + dirInfo.GetDirectories().Length) != 0)
                        {
                            ...
                            {
                                HalconDotNet.HObject Imgtmp;
                                //读取训练图像
                                HalconDotNet.HOperatorSet.ReadImage(out Imgtmp, filePath + "\\TrainImage" + extension);
                                SetTrainImage(Imgtmp);
                            }
                            ...
                            {
                                //读取模板提取区域
                                HalconDotNet.HOperatorSet.ReadRegion(out _modelExtrackRegion, filePath + "\\ExtrackRegion" + extension);
                            }
                            ...
                            {
                                //读取模板搜索区域
                                HalconDotNet.HObject extmp;
                                HalconDotNet.HOperatorSet.ReadRegion(out extmp, filePath + "\\SearchRegion" + extension);
                                ModelSearchRegion = extmp;
                            }
                        }

                        //2--加载模板位姿
                        HalconDotNet.HOperatorSet.FileExists(filePath + "\\ModelPose.tup", out fileExist);
                        if (fileExist.TupleNotEqual(new HalconDotNet.HTuple(0)))
                        {
                            //模板位姿(更新给模板匹配助手)
                            HalconDotNet.HTuple mdlP;
                            HalconDotNet.HOperatorSet.ReadTuple(filePath + "\\ModelPose.tup", out mdlP);
                            ModelPose = mdlP;

                            string modelFile = string.Empty;
                            switch (ModelPose[3].I)
                            {
                                case 0:
                                    ModelType = ProVision.Communal.MatchModelType.ShapeRegionModel;
                                    modelFile = "\\ModelFile.shm";
                                    break;
                                case 1:
                                    ModelType = ProVision.Communal.MatchModelType.ShapeContourModel;
                                    modelFile = "\\ModelFile.shm";
                                    break;
                                case 2:
                                    ModelType = ProVision.Communal.MatchModelType.NCCModel;
                            ...
                            HalconDotNet.HOperatorSet.FileExists(filePath + modelFile, out fileExist);
                            if (fileExist.TupleNotEqual(new HalconDotNet.HTuple(0)))
                            {
                                switch (ModelType)
                                {
                                    ...
                                    case Communal.MatchModelType.ShapeRegionModel:
                                    case Communal.MatchModelType.ShapeContourModel:
                                        HalconDotNet.HOperatorSet.ReadShapeModel(filePath + modelFile, out mdl);
                                        ModelID = mdl;
